...
extension MonthBillList {
    var list: some View {
        ScrollView(showsIndicators: false) {
            ForEach(Array(AppStore.inst.bills.enumerated()), id: \.offset) { index, bill in
                monthBillRowBuilder(bill)
                    .onTapGesture {
                        AppViewModel.inst.homeViewModel.switchToMonth(year: bill.year, month: bill.month)
                        presentationMode.wrappedValue.dismiss()
                    }
                if index != AppStore.inst.bills.count - 1 {
                    Divider()
                }
            }
        }
