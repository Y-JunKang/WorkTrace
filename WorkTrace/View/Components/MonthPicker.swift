...
struct MonthPicker: View {
    ...
    init(display: Binding<Bool>, year: Int, month: Int, onSuccess: @escaping (Int, Int, Int?) -> Void) {
        _display = display
        self.year = year
        self.month = month
        day = nil
    ...
    init(display: Binding<Bool>, year: Int, month: Int, day: Int?, onSuccess: @escaping (Int, Int, Int?) -> Void) {
        _display = display
        self.year = year
        self.month = month
        self.day = day
...
struct MonthPickerPicker_Previews: PreviewProvider {
    static var previews: some View {
        MonthPicker(display: Binding.constant(true), year: 2023, month: 4, onSuccess: { _, _, _ in

        })
        .environmentObject(AppViewModel.debug)
        MonthPicker(display: Binding.constant(true), year: 2023, month: 4, day: 13, onSuccess: { _, _, _ in
